...
mod HueFi {
    ...
    fn generate_random_color(nonce: u64, player: ContractAddress, stake: u256) -> u8 {
        ...
        let player_low: u64 = player.into();
        let stake_low: u64 = stake.low.into();
        let combined = nonce + player_low + stake_low;
        (combined % 5).into()
    }
    ...
    impl HueFiImpl of IHueFi<ContractState> {
        ...
        fn play(
            ...
            // ---------- Validations ----------
            assert(chosen_color < 5_u8, 'Invalid color');
            ...
            self.last_result.write(player, if won { 1_u8 } else { 0_u8 });
